...
{% macro normalize_priority(col) %}
    case
        -- Nhóm 1: đã là số và nằm đúng miền 1..4 của contract -> giữ nguyên.
        when try_cast({{ col }} as integer) between 1 and 4
            then try_cast({{ col }} as integer)
        -- Nhóm 2: nhãn chữ từ 2026-08-10 (schema evolution) -> quy về số.
...
{% macro priority_reject_reason(col) %}
    case
        ...
        else 'priority là chuỗi lạ, không có trong bảng quy đổi'
    end
{% endmacro %}
